...
public class playerexprement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            bool isRight = Input.GetAxisRaw("Horizontal") == 1;
            spriteRenderer_Head.flipX = isRight;
            spriteRenderer_Hair.flipX = isRight;
            spriteRenderer_Body.flipX = isRight;
            spriteRenderer_Arm.flipX = isRight;
            
            // 팔 위치 조정
            if (isRight && arm) 
            {
                // 오른쪽으로 이동할 때 팔의 위치를 조정
                arm.transform.localPosition = new Vector3(-0.14f, arm.transform.localPosition.y, arm.transform.localPosition.z);
            }

            else if (!isRight && arm)
            {
                // 왼쪽으로 이동할 때 팔의 원래 위치로 복귀
                arm.transform.transform.localPosition = new Vector3(0.079f, arm.transform.transform.localPosition.y, arm.transform.transform.localPosition.z);
            }
        }
        //애니메이션 전환
